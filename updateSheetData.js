...
async function updateSheetData() {
    try {
        await pb.admins.authWithPassword(process.env.POCKETHOST_EMAIL, process.env.POCKETHOST_PASSWORD);
        
        console.log(`PocketBase SDK version: ${PocketBase.version || 'Not available'}`);
...
async function updateData(rows) {
        ...
        try {
            ...
            if (existingRecords.length > 0) {
                ...
                if (JSON.stringify(newData) !== JSON.stringify({
                    ...
                    await pb.collection(COLLECTION_NAME).update(existingRecord.id, newData);
                    updatedCount++;
                } else {
                    unchangedCount++;
                }
    ...
    progressBar.stop();
    console.log(`Updated ${updatedCount} records, inserted ${insertedCount} new records, ${unchangedCount} records unchanged`);
